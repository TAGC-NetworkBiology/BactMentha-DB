...
The folder **`01_Reference`** also contain a subfolder **`02_WhoAnnotations`** itself containing a text file with manual currations of WHO priority and hazard group for the studied pathogenes. The reference **`03_AF3_predictions`** contains the interaction complexes predicted using AlphaFold3 via MassiveFold and the **`04_Cellular_Component`** folder contains the UniProt subcellular localizations of all BactMentha proteins when available.
...
The folder **`03_Script`** contains two subfolders. The first one, `01_db_creation_and_update` cotains the python script used for the database creation, data insertion and update. The second folder, `02_Website`, contains the website scripts.  
...
└── bm_archive_2025_08_27
    ├── bactmentha_2025_08_27_complete_archive.zip
    ├── bm_2025_08_27_backup.dmp
    ├── bm_archive_2025_08_27_db_tables_10090.zip
    ├── bm_archive_2025_08_27_db_tables_10116.zip
    ├── bm_archive_2025_08_27_db_tables_9606.zip
    ├── bm_archive_2025_08_27_db_tables_complete.zip
    ├── bm_archive_2025_08_27_ref_af3_predictions.zip
    ├── bm_archive_2025_08_27_ref_complete.zip
    ├── bm_archive_2025_08_27_ref_tables_10090.zip
    ├── bm_archive_2025_08_27_ref_tables_10116.zip
    ├── bm_archive_2025_08_27_ref_tables_9606.zip
    ├── bm_archive_2025_08_27_ref_tables.zip
    └── bm_archive_2025_08_27_ref_WHO_and_CC_annot.zip
```  

Here is the content of the archive .zip files: 
- 1) *bactmentha_2025_08_27_complete_archive.zip* contains:
...
- 2) *bm_archive_2025_08_27_db_tables_XXXX.zip* : contains the part of the database tables that only concern the host taxon XXXX for the specified database version.
- 3) *bm_archive_2025_08_27_db_tables_complete.zip* : contains the complete database tables (all host taxa) for the specified database version. 
- 4) *bm_archive_2025_08_27_ref_tables_XXXX.zip* : contains the reference files used to create the database tables that only concern the host taxon XXXX for the specified database version.
- 5) *bm_archive_2025_08_27_ref_complete.zip* : contains all the reference files used to create the database tables (all host taxa) for the specified database version. 
- 6) *bm_archive_2025_08_27_ref_af3_predictions.zip* : contains only the AlphaFold3 predicted complexes available for the specified version of the database.
- 7) *bm_archive_2025_08_27_ref_WHO_and_CC_annot.zip* : contains only the WHO priority and Hazard groups annotations for bacterial pathogens as well as the proteins UniProt subcellular localization when available.
- 8) *bm_2025_08_27_backup.dmp* : contains the dump file provided to restore the full database with the tables constraints and relations.
...
│       └── Readme.md
├── 03_Script
|   ├── 01_db_creation_and_update
|   │   ├── bactmentha_db_tools
|   │   │   ├── __init__.py
|   │   │   ├── af3_contacts_batch.sh
|   │   │   ├── af3_interactome3D_processing.sh
|   │   │   ├── af3_out_cif_to_pdb.sh
|   │   │   ├── af3_processing.py
|   │   │   ├── computeMutationsStats.py
|   │   │   ├── db_tasks.py
|   │   │   └── get_cellular_components.py
│   │   └── residue_contacts_Interactome3D
│   ├── __init__.py
│   ├── constants.py
│   ├── createdb.py
│   ├── createStatGraphs.py
...
│   ├── bactmentha_advSearch.php
│   ├── bactmentha_contact.php
│   ├── bactmentha_data.php
│   ├── bactmentha_doc.php
│   ├── bactmentha_download.php
│   ├── bactmentha_faq.php
│   ├── bactmentha_home.php
...
│   │   │   └── institutions
│   │   ├── js
│   │   │   └── bactmenthaDB_library.js
│   │   ├── php
│   │   │   └── bactmenthaDB_library.php
│   │   └── tmp
│   └── templates
└── psycopg2_log_error.txt
├── 05_Output
│   ├── 01_Database
│   └── 02_Archive
└── Readme.md
...
- The `sections 2, 4, and 5 have to be modified` by replacing `'Your/Path/To/.../BactMentha_DB'` by your actual path to the project folder.  
- You `don't have to modify sections 3` unless you have changed the name of the mounted volume for python inside the compose_postgres_python.yml file.  
...
*NB : for the `latest` versions:*
    - *downloaded in date `April 2023`. for Php, Adminer and Postgresql images*
    - *downloaded in date `August 2026`. for Python image*
...
The BactMentha Database data can be downloaded directly on the **BactMentha Download page** at `https://bactmentha.tagc.univ-amu.fr/Download` or the **Zenodo** Page at `https://doi.org/10.5281/zenodo.22769706`. 
This include:
- reference tables files (Both Webiste and Zenodo)
- reference proteins and strains annotations (Both Webiste and Zenodo)
- reference AlphaFold complexes (Both Webiste and Zenodo)
- Database formated tables (Both Webiste and Zenodo)
- POSTGRESQL command file to rercreate the database (Both Webiste and Zenodo)
- The specific archives of the database tables splitted by taxa and specific references subpart (Website only as it is redundant with the data provided on Zenodo)
