...
class createGraphs():
    ...
    def __init__(self, path_to_createdb:str, db_name:str, db_host_name:str, db_user_name:str, db_pw:str, db_port:str) -> None:
        """
    ...
    def main(self, cdb:__module__) -> None:
        """
    ...
    def getSatsView(self) -> pd.DataFrame:
        ...
        """
        statsQuery = "SELECT * FROM view_graphical_stats ORDER BY taxon;"
        cursor = self.connection.cursor()
        cursor.execute(statsQuery)
        column_names = [desc[0] for desc in cursor.description]
        records = cursor.fetchall()
        cursor.close()
        return pd.DataFrame(records, columns=column_names, index=None)
    ...
    def getBactStatsViews(self, taxon_name:str) -> pd.DataFrame:
        ...
        """
        bact_stats_query = self.create_bact_stats_query(taxon_name)
        cursor = self.connection.cursor()
        cursor.execute(bact_stats_query)
        column_names = [desc[0] for desc in cursor.description]
        records = cursor.fetchall()
        cursor.close()
        return pd.DataFrame(records, columns=column_names, index=None)
    ...
    def getGraphsSavePaths(self) -> None:
        ...
        Return: None
        """
        common_path = "/BactMentha/03_Script/04_Website/static/img/graphs/"
