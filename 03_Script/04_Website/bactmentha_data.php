    ...
    <form name="view_choice" id="view_choice_form">
        ...
        <div class='browse_text_div'>
            <label class='default_text'>
                ...
                <i>/!\ Loading the Human and All taxa tables can take a few seconds. Thanks for your comprehension.</i>
            </label>
        </div><br>
        ...
        <div class='browse_text_div'>
            <label class='default_text'>
                ...
                Click on the following buttons to filter the BactMentha Datasets and display only the interactions involving
        ...
        <div class='browse_div_titles'>BactMentha dataset filtered on Human-Bacteria annotated protein interactions:</div><br><br>
        <div class='browse_text_div'>
            <label class='default_text'>
                Experimental data have been recovered on some protein-protein interactions, which permit to get more details
                on the binding regions on either host protein side, pathogen protein side or both. The <a class='table_link' 
                href='https://www.ebi.ac.uk/ols4/' target="_blank">Ontology Search (OS)</a> definition of those binding regions 
                can be found <a class='table_link' href='https://www.ebi.ac.uk/ols4/ontologies/mi/classes/http%253A%252F%252Fpurl.obolibrary.org%252Fobo%252FMI_0117?lang=en' 
                target="_blank">here</a>. Moreover, <a class='table_link' href='https://github.com/TAGC-NetworkBiology/mimicINT' 
                target="_blank">mimicINT</a> tool have been used to infer some interaction interfaces between Human proteins 
                and bacterial proteins based on known domain-domain or domain-motif templates of interaction. Click on the 
                following buttons to display only the Human-Bacteria protein-protein interactions for which there are known 
                experimental binding regions (BR) on at least one protein side or mimicINT predicted interfaces (MI) of interaction 
                (always both sides).
            </label>
        </div><br>
        <div class='home_buttons_space'>
      ...
      <?php
        ...
        if (!empty($table_name) && $_SESSION["data_view_type"] != "who_priority") {
            if ($_SESSION["data_view_type"] == "who_interaction_data") {
                ...
                $psql_query = browsePageCreationQuery($table_name) . " WHERE IF.annotations LIKE 't__'";
            } else if ($_SESSION["data_view_type"] == "br_interaction_data") {
                $psql_query = browsePageCreationQuery($table_name) . " WHERE IF.annotations LIKE '_t_'";
            } else {
...
<script>
    ...
    $(document).ready(function () {
        ...
        if ($('#query_table').length) {
            ...
            $('#query_table tbody').on('click', '.moreInfoBtnLinkBlue', function() {
                ...
                if (row.child.isShown() && tr.hasClass('bluechild')) {
                    ...
                    // Open this row
                    row.child(format_blue(row.data(), bmIdIndex)).show();
                    tr.addClass('shown');
            ...
            $('#query_table tbody').on('click', '.moreInfoBtnLinkGreen', function() {
                var tr = $(this).closest('tr');
                var row = table.row(tr);
                if (row.child.isShown() && tr.hasClass('greenchild')) {
                    // This row is already open - close it
                    ...
                    // Open this row
                    row.child(format_green(row.data(), bmIdIndex)).show();
                    tr.addClass('shown');
            ...
            $('#query_table tbody').on('click', '.moreInfoBtnLinkYellow', function() {
                var tr = $(this).closest('tr');
                var row = table.row(tr);
                if (row.child.isShown() && tr.hasClass('yellowchild')) {
                    ...
                    // Open this row
                    row.child(format_yellow(row.data(), bmIdIndex)).show();
                    tr.addClass('shown');
