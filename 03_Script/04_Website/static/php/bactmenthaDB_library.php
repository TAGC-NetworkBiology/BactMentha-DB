...
function findViewWithTypeAndTaxon($getViewType, $getTaxon) {
    ...
    $listViewTypes = array("interaction_data" => "A", "bacterial_interaction_stats" => "B",
     ...
     "br_interaction_data" => "H", "hg3_interaction_data" => "I");
...
function findLastUpdate($dbconn) {
    ...
    $fullDate = explode("_", $result);                                                  // split the date with the underscores
    $year = $fullDate[0];                                                               // find the year
    $months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
    $month = $months[intval($fullDate[1])-1];                                                     // find the month
    $day_nb = intval($fullDate[2]);                                                               // find the day number
    ...
    $date = $month." ".$day_nb.$day_ext." ".$year;
    return $date ;
}
...
function dispMimicintChildRow($bmId) {
    ...
    $childRowTable = writeMimicintChildTable($query_result, 'display stripe child_table_blue');                      // child table creation
...
function dispInteractionFeatureChildRow($bmId) {
    ...
    $query_result = pg_query($psql_query) or die("\nError in the query execution : ".pg_last_error());  // query execution
    $close=pg_close($dbconn);                                                                           // closing postgresql connection
    $childRowTable = writeFeaturesChildTable($query_result, 'display stripe child_table_green');                      // child table creation
    return $childRowTable ;                                                                             // return the html table in a string
}
...
function dispProtAnnotationChildRow($bmId) {
    ...
    $query_result = pg_query($psql_query) or die("\nError in the query execution : ".pg_last_error());  // query execution
    $close=pg_close($dbconn);                                                                           // closing postgresql connection
    $childRowTable = writeProtAnnotChildTable($query_result, 'display stripe child_table_yellow');                    // child table creation
