...
class Createdb():
    ...
    def __init__(self, db_name:str, db_host_name:str, db_user_name:str, db_pw:str, db_port:str) -> None:
        ...
        self.connection = self.create_server_connection(db_name, db_host_name, db_user_name, db_pw, db_port)
        ...
        create_metedata_query = """CREATE TABLE IF NOT EXISTS metadata (
        ...
        query_list = [create_uniprot_taxonomy_query,
                      ...
                      create_metedata_query]
        # _____________________________________________
        # EXECUTION OF THE QUERIES TO CREATE THE TABLES
        self.tables_creation(query_list)
        # _____________________________________
        ...
        view_bact_annotation_count_global = """CREATE OR REPLACE VIEW view_bact_annotation_count_global AS
                                        SELECT 
                                            ...
                                            SUM(CASE WHEN SUBSTRING(IF.annotations, 3, 1) = 't' THEN 1 ELSE 0 END) AS number_of_annotated_interactions
                                        FROM 
        ...
        # EXECUTION OF THE QUERIES TO CREATE THE VIEWS
        self.views_creation(view_list)
        print("\nEnd of script createdb.py. Closing connexion to PosgreSQL database.\n")
        self.connection.close()
 

    def tables_creation(self, query_list:list) -> None:
        """
        Creation of the database tables using pre-defined queries.

        Args:
            - query_list: a list of psql queries to execute to create the database tables
    ...
    def views_creation(self, view_list:list) -> None:
        """
        Creation of the view for the website using pre-defined queries.

        Args:
            - view_list: a list of queries to execute to create the database view

        Return: None
        """
        print("##### CREATING THE DATABASE VIEWS #####")
        for view_query in view_list:
            self.execute_query(self.connection, view_query, self.state)
        print("Successfully created the database views")

    @staticmethod
    def create_server_connection(db_name:str, db_host_name:str, db_user_name:str, db_pw:str, db_port:str) -> object:
        """
    ...
    @staticmethod
    def execute_query(connection: object, query: str) -> None:
        """
        ...
        try:
            ...
            if "current transaction is aborted" not in str(e):
                with open("/BactMentha/03_Script/psycopg2_log_error.txt", 'a') as log:
                    log.write(f"Error while executing query:\n{query}\nError details: {e}\n\n")
                print(f"Error logged in /BactMentha/03_Script/psycopg2_log_error.txt")
...
if __name__ == "__main__":
    ...
    Createdb(args.db_name[0], args.db_host_name[0], args.db_user_name[0], args.db_password[0], args.db_port[0])
